...
struct PositionDetailView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if isLoading {
                ProgressView("Loading…").foregroundStyle(Color.appSubtext)
            } else {
                ScrollView {
                    VStack(spacing: 16) {
                        headerCard
            ...
            HStack(alignment: .bottom) {
                ...
                if let pct = detail?.changePct {
                    VStack(alignment: .trailing, spacing: 4) {
                        ...
                        Text("today").font(.caption2).foregroundStyle(Color.appSubtext)
                    }
                }
            }
    ...
    private var statsGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
            ...
            gridCell(label: "Realized", value: position.realizedPnl.currency,
                     color: position.realizedPnl.gainColor)
        }
    }
    ...
    private func aboutCard(_ text: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("About").font(.caption.bold()).foregroundStyle(Color.appSubtext)
            Text(text).font(.caption).foregroundStyle(Color.appSubtext).lineLimit(6)
        }
    ...
    private var sellButton: some View {
        Button { showSell = true } label: {
            HStack {
                ...
                Text("Sell \(position.ticker)").font(.headline)
            }
            .frame(maxWidth: .infinity).padding(14)
            .background(Color.appLoss).foregroundStyle(Color.white)
...
// MARK: - Sell sheet (no NavigationStack to avoid toolbar ambiguity)
...
struct SellFromDetailSheet: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                // Manual header (avoids toolbar ambiguity)
                HStack {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        if !errorMsg.isEmpty {
                            Text(errorMsg).font(.caption).foregroundStyle(Color.appLoss)
                        ...
                        Button(action: { Task { await submit() } }) {
                            HStack {
                                ...
                                Text(isSubmitting ? "Submitting…" : "Confirm Sell").font(.headline)
                            }
                            .frame(maxWidth: .infinity).padding(14)
    ...
    private var sharesField: some View {
        VStack(alignment: .leading, spacing: 6) {
            rowLabel("Shares to Sell")
            TextField("0", text: $sharesStr).keyboardType(.decimalPad)
                .foregroundStyle(Color.appText).sellFieldStyle()
    ...
    private func rowLabel(_ text: String) -> some View {
        Text(text).font(.caption.bold()).foregroundStyle(Color.appSubtext).padding(.leading, 20)
    }
